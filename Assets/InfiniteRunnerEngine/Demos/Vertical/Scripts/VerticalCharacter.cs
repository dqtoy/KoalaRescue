...
{	
	...
	{		
		...
		{
			if ((_rigidbodyInterface.Velocity.y <= 0) && (_rigidbodyInterface.ColliderBounds.min.y >= collidingObject.GetComponentNoAlloc<Collider2D>().bounds.max.y))
			{
		...
		protected override void UpdateAllMecanimAnimators()
		{
			...
			MMAnimator.UpdateAnimatorFloatIfExists(_animator, "HorizontalSpeed", Mathf.Abs(_rigidbodyInterface.Velocity.x));
			MMAnimator.UpdateAnimatorBoolIfExists(_animator, "WallSliding", _wallsliding);
			MMAnimator.UpdateAnimatorBoolIfExists(_animator, "Grounded", _verticalGrounded);
